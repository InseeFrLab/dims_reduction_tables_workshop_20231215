...
load("res_simulations_paper5.RData")
load("res_simulations2_paper5.RData")
load("res_simulations5_paper5.RData")
...
res_simulations2 %>% select(method, pc_val_secondary) %>% mutate(prim_thq = 10) %>% 
  bind_rows(
    res_simulations2 %>% select(method, pc_val_secondary) %>% mutate(prim_thq = 20)) %>%
  bind_rows(
    res_simulations5 %>% select(method, pc_val_secondary) %>% mutate(prim_thq = 50)) %>% 
...
ggsave(filename = "boxplot_sims4dims_withhier_nodirecthyper.pdf", device = "pdf", units = "cm", width = 20, height = 8)
